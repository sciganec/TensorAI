import StateK from './StateK.js';

export class QuadrotorOperators {
  ...
  static mask(stateK, mask) {
    ...
    let result = '';
    for (let i = 0; i < 6; i++) result += (parseInt(stateK.k[i]) ^ parseInt(mask[i])).toString();
  ...
  static findPath(startK, targetK, maxEnergy = 10) {
    ...
    while (queue.length > 0) {
      ...
      if (state.k === targetK.k) return { success: true, path, energy, finalState: state };
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (!visited.has(newState.k) && newEnergy <= maxEnergy) {
          ...
          queue.push({ state: newState, path: [...path, { op: `F(${i})`, from: state.k, to: newState.k }], energy: newEnergy });
        }
      }
    }
...
export default QuadrotorOperators;
