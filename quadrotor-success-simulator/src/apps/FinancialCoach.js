const { StateK } = require('../core/StateK');
...
class FinancialCoach {
  ...
  // Аналіз поточного стану для фінансових рішень
  analyzeFinancialState() {
    ...
    return {
      currentState: this.currentState.k,
      hexagram: this._getHexagram(this.currentState.k),
      advice,
      recommendedTransitions: this._getFinancialTransitions()
    };
  }
  ...
  _getFinancialTransitions() {
    ...
    return transitions[this.currentState.k] || 
      { target: '111111', operator: 'F(0)&F(2)&F(4)', goal: 'Загальний шлях до процвітання' };
  }
